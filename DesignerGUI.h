...
void saveCallback(Fl_Widget* widget, void* data)
{
    std::ofstream outFile("testSave.txt"); // Open the output file

    int i = 0;
    for (std::string& mapName : thisSession.mapNames)
    {
        // Write map name
        ...
        outFile << thisSession.worldMaps.at(i)->mapPlayerLocationX << std::endl;
        outFile << "[playerY]" << std::endl;
        outFile << thisSession.worldMaps.at(i)->mapPlayerLocationY << std::endl;

        // Write newMap
        outFile << "[newMap]" << std::endl;
        for (int pos : thisSession.worldMaps.at(i)->newMap)
        {
            outFile << pos << ",";
        }
        outFile << "\n" << '#' << std::endl;
        ...
        // Write ceilingMap
        outFile << "[ceilingMap]" << std::endl;
        for (int pos : thisSession.worldMaps.at(i)->ceilingMap)
        {
            outFile << pos << ",";
        }
        outFile << "\n" << '#' << std::endl;
        ...
        // Write floorMap
        outFile << "[floorMap]" << std::endl;
        for (int pos : thisSession.worldMaps.at(i)->floorMap)
        {
            outFile << pos << ",";
        }
        outFile << "\n" << '#' << std::endl;
        ...
        for (const std::vector<int>& position : thisSession.worldMaps.at(i)->positions)
        {
        ...
        // Write spriteData
        outFile << "[spriteData]" << std::endl;
        for (sprite Sprite : thisSession.worldMaps.at(i)->spriteData)
        {
            outFile << "Type: " << Sprite.type << " State: " << Sprite.state << " Map: " << Sprite.map
                << " X: " << Sprite.x << " Y: " << Sprite.y << " Z: " << Sprite.z
                << " DialogueMarker: " << Sprite.dialogueMarker << " DialogueGraphic: " << Sprite.dialogueGraphic << std::endl;
        }
        outFile << "#" << std::endl;
...
void loadDataCallback(Fl_Widget* widget, void* data)
{
    gameState = FREEZE;
    std::ifstream inFile("testSave.txt"); // Open the input file
    ...
    sessionData loadedSession; // Create a temporary sessionData to load data into


    int playerX = 0;
    int playerY = 0;

    // Temporary vector to store map data
    std::vector<std::vector<int>> positionsData; // Temporary vector to store positions data
    sprite Sprite; // Temporary sprite structure to store sprite data
    ...
    while (std::getline(inFile, line))
    {
        ...
        if (line == "[mapName]")
        {
            ...
            std::cout << "mapname: " << line << std::endl;

            loadedSession.mapNames.push_back(line);

            std::getline(inFile, line); //fetch next line

        }
        if (line == "[playerX]")
        {
            std::getline(inFile, line);
            std::cout << "PlayerX: " << line << std::endl;
            mapData->mapPlayerLocationX = std::stoi(line);
        ...
        if (line == "[playerY]")
        {
            ...
            mapData->mapPlayerLocationY = std::stoi(line);
        ...
        if (line == "[positions]")
        {
            ...
            while (line != "#")
            {
                ...
                positionsData.push_back(position);
               std::getline(inFile, line);
               if (line == "#")
               {
            ...
            mapData->positions = positionsData;
            std::getline(inFile, line); //fetch next line
        }
        if (line == "[spriteData]")
        {

            std::vector<sprite> spriteData; // Temporary vector to store sprite data
            std::cout << line << std::endl;
            std::getline(inFile, line);
            ...
            while (line != "#")
            {
                ...
                while (iss >> val)
                {
                    ...
                    else if (val == "Z:")
                    {
                        ...
                        iss >> val;
                        std::cout << val << "-";
                        Sprite.z = std::stoi(val);
                    
                    }
                    else if (val == "DialogueMarker:")
                    {
                        iss >> val;
                        std::cout << val << "-";
                        Sprite.dialogueMarker = std::stoi(val);
                    
                    
                    }
                    else if (val == "DialogueGraphic:")
                    {
                        iss >> val;
                        std::cout << val << "-";
                        Sprite.dialogueGraphic = std::stoi(val);
            ...
            mapData->spriteData = spriteData;
        }

       


        loadedSession.worldMaps.push_back(mapData);
    ...
    Fl_Box* label_holder = thisSession.mapLabel;
    thisSession = loadedSession;
    thisSession.mapLabel = label_holder;
    ...
    for (std::string& name : thisSession.mapNames)
    {
        std::cout << "mapName: " << name << std::endl;


    }
    for (MapData* mapData : thisSession.worldMaps)
    {

        std::cout << "X: " << mapData->mapPlayerLocationX << " Y: " << mapData->mapPlayerLocationY << std::endl;
        std::cout << "newMap: " << mapData->newMap[575] << std::endl;

        

    }
    std::cout << "currentmap = " << thisSession.currentMap << std::endl;
...
void mapForwardCallback(Fl_Widget* widget, void* data)
{
    if (thisSession.currentMap + 1 < thisSession.worldMaps.size())
    {
        ...
        thisSession.mapLabel->label(thisSession.mapNames.at(thisSession.currentMap).c_str());
...
void designerGui()
{
    ...
    //
    Fl_Button* Texture = new Fl_Button(600, 294, 120, 30, "Texture");
    ...
    //SKY
    Fl_Box* Skylabel = new Fl_Box(600, 354, 120, 30, "SKY:");
    ...
    //Wall
    Fl_Box* wallLabel = new Fl_Box(600, 384, 120, 30, "WALL:");
    ...
    //Floor
    Fl_Box* floorLabel = new Fl_Box(600, 324, 120, 30, "FLOOR:");
    ...
    //save and load
   Fl_Button* saveButton = new Fl_Button(960, 174, 120, 30, "Nsave");
    saveButton->callback(saveCallback);

    Fl_Button* loadButton = new  Fl_Button(960, 204, 120, 30, "load");
    loadButton->callback(loadDataCallback);

    Fl_Button* DeleteMap = new  Fl_Button(960, 234, 120, 30, "Delete");

    Fl_Button* loadSprites = new Fl_Button(960, 264, 120, 30, "loadSprite");
